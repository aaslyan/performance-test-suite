...
BenchmarkResult IntegratedBenchmark::run(int duration_seconds, int iterations, bool verbose) {
    ...
    try {
        ...
        result.throughput = (net_mem.throughput_ops_sec + mem_disk.throughput_ops_sec + 
                           full_pipeline.throughput_ops_sec) / 3.0;
        result.throughput_unit = "ops/sec";
        
        result.avg_latency = (net_mem.end_to_end_latency_ms + mem_disk.end_to_end_latency_ms +
                            full_pipeline.end_to_end_latency_ms) / 3.0;
